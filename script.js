...
let buttonShow = document.getElementsByClassName('product__show')[0];
let eachAfterEight = document.querySelectorAll(".product__box:nth-child(n+9)")

for (var i = 0; i < eachAfterEight.length; i++) {
  
    eachAfterEight[i].classList.add('product__none');

}

buttonShow.addEventListener('click', function () {

  let eachAfter = document.querySelectorAll(".product__box:nth-child(n+9)")

  for (var i = 0; i < eachAfter.length; i++) {
  
      //  делаем проверку на наличие класса
      if (eachAfter[i].classList.contains('product__none')) {
        // если класс есть, удаляем его
        eachAfter[i].classList.remove('product__none');

  
      } else {
        // если класса нет, добавляем его
        eachAfter[i].classList.add('product__none');
      }
  
  }

  //  делаем проверку на текст
  if (buttonShow.innerHTML == 'Показать еще') {
    buttonShow.innerHTML = "Скрыть";
  } else {
    buttonShow.innerHTML = "Показать еще";
  }
